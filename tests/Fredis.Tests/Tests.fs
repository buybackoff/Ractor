...
[<AutoOpenAttribute>]
module Helper =
    let greeterComputation input =  
        async {
                Console.WriteLine("Hello, " + input)
                return "Hello, " + input
        } |> Async.StartAsTask
    let echo input =  
        async { return input } |> Async.StartAsTask
...
type Greeter ()=
    ...
    override this.Computation(input) = greeterComputation input
...
type Echo ()=
    inherit Fredis.Actor<string,string>()
    override this.Redis = "localhost"
    override this.Computation(input) = echo input
    //override this.AutoStart = false
...
let ``Hello, Actors`` () =
    ...
    Greeter().Post("Greeter nice syntax") |> ignore
